...
class User_privileges extends Admin_Controller {
	...
	public function index() {
		...
		$this->vars['sub_title'] = 'Hak akses modul untuk administrator';
		...
		$this->vars['module_dropdown'] = json_encode($this->m_modules->dropdown(), JSON_HEX_APOS | JSON_HEX_QUOT);
		$this->vars['content'] = 'users/user_privileges';
	...
	public function pagination() {
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->m_user_privileges->get_where($keyword, 'rows', $limit, $offset);
			$total_rows = $this->m_user_privileges->get_where($keyword);
	...
	private function dataset() {
		return [
			'user_group_id' => _toInteger($this->input->post('user_group_id', true)),
			'module_id' => _toInteger($this->input->post('module_id', true))
		];
	}
