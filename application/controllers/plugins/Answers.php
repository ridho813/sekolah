...
class Answers extends Admin_Controller {
	...
	public function __construct() {
		parent::__construct();
		$this->load->model(['m_answers', 'm_questions']);
		$this->pk = M_answers::$pk;
		$this->table = M_answers::$table;
	}
	...
	public function index() {
		$this->vars['title'] = 'Jawaban';
		$this->vars['plugins'] = $this->vars['pollings'] = $this->vars['answers'] = TRUE;
		$this->vars['parent_data'] = json_encode($this->m_questions->dropdown(), JSON_HEX_APOS | JSON_HEX_QUOT);
		$this->vars['content'] = 'plugins/answers';
	...
	public function pagination() {
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->m_answers->get_where($keyword, 'rows', $limit, $offset);
			$total_rows = $this->m_answers->get_where($keyword);
	...
	private function dataset() {
		return [
			...
			'question_id' => _toInteger($this->input->post('question_id', true))
		];
	}
	...
	private function validation() {
		...
		$val->set_rules('answer', 'Answer', 'trim|required');
		$val->set_rules('question_id', 'Question', 'trim|required');
