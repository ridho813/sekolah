...
class Majors extends Admin_Controller {
	...
	public function index() {
		$this->vars['title'] = ucwords(strtolower(__session('_major')));
		$this->vars['academic'] = $this->vars['academic_references'] = $this->vars['majors'] = TRUE;
		$this->vars['content'] = 'academic/majors';
	...
	public function pagination() {
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->m_majors->get_where($keyword, 'rows', $limit, $offset);
			$total_rows = $this->m_majors->get_where($keyword);
	...
	private function dataset() {
		return [
			...
			'major_short_name' => $this->input->post('major_short_name', true),
	...
	private function validation() {
		...
		$val->set_rules('major_name', __session('_major'), 'trim|required');
		$val->set_rules('major_short_name', 'Nama Singkat', 'trim|required');
		$val->set_rules('is_active', 'Tampil di PPDB / PMB', 'trim|required|in_list[true,false]');
