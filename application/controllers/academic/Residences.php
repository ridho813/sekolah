...
class Residences extends Admin_Controller {
	...
	public function index() {
		$this->vars['title'] = 'Tempat Tinggal';
		$this->vars['academic'] = $this->vars['academic_references'] = $this->vars['residences'] = TRUE;
		$this->vars['content'] = 'academic/residences';
	...
	public function pagination() {
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->m_options->get_where($keyword, 'residences', 'rows', $limit, $offset);
			$total_rows = $this->m_options->get_where($keyword, 'residences');
	...
	private function validation() {
		...
		$val->set_rules('option_name', 'Tempat Tinggal', 'trim|required');
