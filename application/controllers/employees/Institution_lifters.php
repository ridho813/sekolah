...
class Institution_lifters extends Admin_Controller {
	...
	public function index() {
		$this->vars['title'] = 'Lembaga Pengangkat';
		$this->vars['employees'] = $this->vars['employee_references'] =$this->vars['institution_lifters'] = TRUE;
		$this->vars['content'] = 'employees/institution_lifters';
	...
	public function pagination() {
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->m_options->get_where($keyword, 'institution_lifters', 'rows', $limit, $offset);
			$total_rows = $this->m_options->get_where($keyword, 'institution_lifters');
	...
	private function dataset() {
		return [
			'option_group' => 'institution_lifters',
	...
	private function validation() {
		...
		$val->set_rules('option_name', 'Lembaga Pengangkat', 'trim|required');
