...
class Ranks extends Admin_Controller {
	...
	public function index() {
		$this->vars['title'] = 'Pangkat/Golongan';
		$this->vars['employees'] = $this->vars['employee_references'] =$this->vars['ranks'] = TRUE;
		$this->vars['content'] = 'employees/ranks';
	...
	public function pagination() {
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->m_options->get_where($keyword, 'ranks', 'rows', $limit, $offset);
			$total_rows = $this->m_options->get_where($keyword, 'ranks');
	...
	private function validation() {
		...
		$val->set_rules('option_name', 'Golongan', 'trim|required');
