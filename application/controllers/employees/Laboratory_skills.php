...
class Laboratory_skills extends Admin_Controller {
	...
	public function index() {
		...
		$this->vars['employees'] = $this->vars['employee_references'] =$this->vars['laboratory_skills'] = TRUE;
		$this->vars['content'] = 'employees/laboratory_skills';
	...
	public function pagination() {
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->m_options->get_where($keyword, 'laboratory_skills', 'rows', $limit, $offset);
			$total_rows = $this->m_options->get_where($keyword, 'laboratory_skills');
	...
	private function dataset() {
		return [
			'option_group' => 'laboratory_skills',
	...
	private function validation() {
		...
		$val->set_rules('option_name', 'Keahlian Laboratorium', 'trim|required');
