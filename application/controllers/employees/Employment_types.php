...
class Employment_types extends Admin_Controller {
	...
	public function index() {
		$this->vars['title'] = 'Jenis '.__session('__employee');
		$this->vars['employees'] = $this->vars['employee_references'] =$this->vars['employment_types'] = TRUE;
		$this->vars['content'] = 'employees/employment_types';
	...
	public function pagination() {
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->m_options->get_where($keyword, 'employment_types', 'rows', $limit, $offset);
			$total_rows = $this->m_options->get_where($keyword, 'employment_types');
	...
	private function validation() {
		...
		$val->set_rules('option_name', 'Jenis ' . (__session('__employee')), 'trim|required');
