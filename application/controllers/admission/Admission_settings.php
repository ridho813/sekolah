...
class Admission_settings extends Admin_Controller {
	...
	public function index() {
		$this->vars['title'] = 'Pengaturan Penerimaan ' . __session('_student') . ' Baru';
		$this->vars['admission'] = $this->vars['admission_settings'] = TRUE;
		$this->vars['content'] = 'admission/admission_settings';
	...
	public function pagination() {
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->m_settings->get_where($keyword, 'admission', 'rows', $limit, $offset);
			$total_rows = $this->m_settings->get_where($keyword, 'admission');
	...
	private function validation() {
		...
		$val->set_rules('setting_value', 'Setting Value', 'trim|required');
